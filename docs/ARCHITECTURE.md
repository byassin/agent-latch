...
Codex JSONL state remains authoritative. When one or more lifecycle tasks are active, a simultaneous Codex composer response does not add another instance. A ChatGPT response or a Codex composer response with no matching JSONL task contributes one active instance and precise dashboard detail. While active, the probe refreshes the same activity timestamp used by the existing grace-period release behavior.
